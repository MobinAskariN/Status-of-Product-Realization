...
using realization_graph_4._5.Models;
using System;
using System.Diagnostics;

namespace realization_graph_4._5.Controllers
{
    public class GraphController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            List<SectionTypeTreeSectionCharts> first_sectionTree = _context.getSectionTypeTreeSectionCharts(int.Parse(first_layer.sectionTypeId));
            ...
            if (first_station.requiredDocId != null)
            {
                ...
                foreach (Vertex v in first_label.vertices)
                {
                    ...
                    List<TreeSectionChartDocuments> treeSectionChartDocuments = _context.getTreeSectionChartDocuments(v.id);
                    foreach (TreeSectionChartDocuments TreeDocument in treeSectionChartDocuments)
                    {
                        ...
                        if (required_doc.Keys.Contains(document.DOCTYPEID))
                        {
                            ...
                            Debug.WriteLine(required_doc[document.DOCTYPEID]);
            ...
            while (true)// stationId >= 2
            {
                ...
                else if (station.layerId > previous_layerId)// jaryan vagara
                {
                    ...
                    {
                        List<SectionTypeTreeSectionCharts> sectionTypeTreeSectionCharts = _context.getSectionTypeTreeSectionCharts(sectionType_ID);
                        number_of_vertices += sectionTypeTreeSectionCharts.Count;

                        for (int i = 0; i < sectionTypeTreeSectionCharts.Count; i++)
                        {
                            TreeSectionCharts treeSectionCharts = _context.getTreeSectionCharts(sectionTypeTreeSectionCharts.ElementAt(i).TreeSectionChart_ID);
                            ...
                            Debug.WriteLine(treeSectionCharts.SectionName);
            ...
            // replacment required for .net 4.5 for edges
            List<Edge> new_edges = new List<Edge>();
            foreach((Vertex v, Vertex u) in edges)
            {
                Edge e = new Edge();
                e.StartVertex = v;
                e.EndVertex = u;
